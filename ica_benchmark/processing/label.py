...
def get_annotations_from_gdf(gdf_obj):
    ...
    durations = np.trunc(data.annotations.duration * sr).astype(
...
def create_null_class(x):
    x = np.concatenate([np.zeros((x.shape[0], 1)), x.copy()], axis=1)
    x[:, 0] = np.bitwise_not(np.any(x[:, 1:], axis=1)).astype(np.uint32)
    return x
