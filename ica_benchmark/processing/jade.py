...
def jadeR(X, n_components=None, apply_pca=True):
    ...
    X = matrix(
        X.astype(float64)
    )  # create a matrix from a copy of X created as a float 64 array

    [n, T] = X.shape

    m = n if n_components is None else n_components
    ...
    CM = matrix(zeros([m, int(m * nbcm)], dtype=float64))
    ...
    while encore:
        ...
        for p in range(m - 1):  # m == 3
            for q in range(p + 1, m):  # p == 1 | range(p+1, m) == [2]
                ...
                Ip = arange(
                    p, m * nbcm, m
                )  # [ 0  3  6  9 12 15] [ 0  3  6  9 12 15] [ 1  4  7 10 13 16]
                Iq = arange(
                    q, m * nbcm, m
                )  # [ 1  4  7 10 13 16] [ 2  5  8 11 14 17] [ 2  5  8 11 14 17]
                ...
                theta = 0.5 * arctan2(
                    toff, ton + sqrt(ton * ton + toff * toff)
                )  # -0.491778606993 -0.194537202087 0.463781701868
                Gain = (
                    sqrt(ton * ton + toff * toff) - ton
                ) / 4.0  # 5.87059352069 0.449409565866 2.24448683877
                ...
                if abs(theta) > seuil:
                    ...
                    G = matrix(
                        [[c, -s], [s, c]]
                    )  # DON"T PRINT THIS! IT"LL BREAK THINGS! HELLA LONG
    ...
    A = pinv(
        B
    )  # [[-3.35031851 -2.14563715  0.60277625] [-2.49989794 -1.25230985 -0.0835184 ] [-2.49501641 -0.67979249  0.12907178]]
...
def jade(X, n_components=None):
    B, _ = jadeR(X, n_components=n_components)
    Y = dot(B, X)
    return Y
