...
def preprocess_epochs(epochs, picks=PICKS, inplace=False):
    if inplace:
        return epochs.load_data().pick(picks).filter(l_freq=None, h_freq=40).resample(90.)

    return epochs.copy().load_data().pick(picks).filter(l_freq=None, h_freq=40).resample(90.)
...
def run_ica_experiment(_run, method_idx):
    ...
    for method in methods:
        ...
        results[method] = list()
        clf = make_pipeline(
            PSD(fmin=0.1, fmax=40, picks=PSD_PICKS),
            Averager(),
            StandardScaler(),
            SVC(
                C=4
