...
from ica_benchmark.processing.dataset import WindowTransformerStaticDataset
...
def eval_dataset(m, loader, loss_fn, run=None, epoch=None, name="D"):
    y_t = []
    y_p = []
    losses = list()
    for x, y in loader:
        pred = m(x.float().cuda())
        loss = loss_fn(pred, y.float().cuda())
        losses.append(loss.item())
        y_p_value = pred.cpu().detach().numpy()
    ...
    run.log_scalar(f"{name}_loss", torch.mean(loss).item(), epoch)
...
#    x_test = ica_transform.transform(x_test)
    ...
    freqs = np.array([4, 8, 10, 12, 16, 20])
    ...
    dataset = WindowTransformerStaticDataset(x_train, y_train, **dataset_kwargs)
    train_dataloader = DataLoader(dataset, **dataloader_kwargs)
    print(len(dataset))
    print("Creating Val dataset")
    dataloader_kwargs.update(dict(batch_size=128))
    dataset = WindowTransformerStaticDataset(x_val, y_val, **dataset_kwargs)
    val_dataloader = DataLoader(dataset, **dataloader_kwargs)   
    print(len(dataset))
    print("Creating Test dataset")
    dataset = WindowTransformerStaticDataset(x_test, y_test, **dataset_kwargs)
    ...
    for epoch in range(30):
        ...
        model.eval()
        val_metric = eval_dataset(model, val_dataloader, loss_fn, run=_run, epoch=epoch, name="val")
    ...
    model.eval()
    eval_dataset(model, test_dataloader, loss_fn, run=_run, epoch=epoch, name="test")
