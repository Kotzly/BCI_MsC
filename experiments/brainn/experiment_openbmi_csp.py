...
def run(dataset, uid, load_kwargs, clf_methods=None, n_runs=10, random_state=1):
    ...
    intra_test_epochs = dataset.load_subject(uid, session=1, run=2, **load_kwargs)[0]
    inter_test_epochs = dataset.load_subject(uid, session=2, run=1, **load_kwargs)[0]

    train_epochs.load_data()
    intra_test_epochs.load_data()
    ...
    channels += ["C" + str(s) for s in [5, 3, 1, "z", 2, 4, 6]]
    channels += ["CP" + str(s) for s in [5, 3, 1, "z", 2, 4, 6]]

    # Filtering
    train_epochs = train_epochs.pick(channels).filter(8, 30, **filter_kwargs)
    intra_test_epochs = intra_test_epochs.pick(channels).filter(8, 30, **filter_kwargs)
    inter_test_epochs = inter_test_epochs.pick(channels).filter(8, 30, **filter_kwargs)
...
if __name__ == "__main__":
    ...
    for uid in dataset.list_uids():
        ...
        results_dfs = run(dataset, uid, load_kwargs, clf_methods=clf_methods, n_runs=N_RUNS)
