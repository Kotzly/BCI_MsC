...
def run(splits, uid, ica_methods=None, clf_methods=None, channels=None, n_runs=10, random_state=1):
    ...
    test_epochs = test_epochs[(test_labels == 0) | (test_labels == 1)]

    # To match the size of the intra-run protocol  (50% to each split), here we would have 1 run for each split
    train_epochs = train_epochs[:len(train_epochs) // 2]
    test_epochs = test_epochs[:len(test_epochs) // 2]
...
if __name__ == "__main__":
    ...
    for i, splits in enumerate(splits_iterable):
        print(splits)
        uid = splits[0]["uid"][0]
        train_session, train_run = np.unique(splits[0]["session"]).item(), np.unique(splits[0]["run"]).item()
        test_session, test_run = np.unique(splits[1]["session"]).item(), np.unique(splits[1]["run"]).item()
        ...
        subject_results_df["test_session"] = test_session
        subject_results_df["train_run"] = train_run
        subject_results_df["test_run"] = test_run
