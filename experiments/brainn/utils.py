...
from mne import BaseEpochs, Epochs
...
def is_ica_stochastic(ica_method):
    return not(
        ("orica" in ica_method) or (ica_method in ("sobi", "jade", "none", None, "pca", "whitening"))
    )
...
def repeat_csp_deterministic(results_df, times=1):
    original_df = results_df.copy()
    results_df = results_df.copy()
    results_df["duplicated"] = 0
    keys = ["classifier"]
    ...
    non_repeated = original_df.groupby(keys, as_index=False).nunique().query("run == 1")
    for _, (clf,) in non_repeated[keys].iterrows():
        for i in range(times):
            sliced_df = original_df.query("(classifier == @clf)").copy()
            if i == sliced_df.run.unique().item():
                continue
            sliced_df.loc[:, ["run"]] = i
            sliced_df.loc[:, ["duplicated"]] = 1
...
def get_PSD(data, sfreq=250, len_size=250):
    if isinstance(data, (BaseEpochs, Epochs)):
        psd = PSD(
            picks=data.ch_names,
            n_fft=1 * len_size,
...
class PSD(BaseEstimator):
    ...
    def transform(self, x, y=None):
        ...
        elif isinstance(x, (mne.BaseEpochs, mne.Epochs)):
            psds, freqs = psd_welch(x, ** self.kwargs)
        elif isinstance(x, np.ndarray):
            psds, freqs = psd_array_welch(x, ** self.kwargs)
        elif ("average" in self.kwargs) and (self.kwargs["average"] is None):
            psds = psds.sum(axis=3)
        else:
            raise Exception("X is not of a known data type")
        self.freqs = freqs
